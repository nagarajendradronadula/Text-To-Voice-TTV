...
MURF_API_KEY = 'ap2_10079d88-9641-4808-8047-ec1aff896b0d'
...
def convert_with_espeak(text, voice, speed):
    ...
    if voice in voice_map:
        ...
        return send_file(mp3_path, as_attachment=True, download_name=filename.replace('.wav', '.mp3'), mimetype='audio/mpeg')
...
def convert_with_murf(text, voice, speed):
    ...
    try:
        ...
        print(f"Murf response: {response.text}")
        ...
        return jsonify({'error': f'Murf API failed: {response.status_code} - {response.text}'}), 500
    except Exception as e:
        print(f"Murf exception: {str(e)}")
